...
#include "librmcs/client/gpio_command.hpp"
...
public:
            ...
            create_partner_component<TestCommand>(get_component_name() + "_command", *this))
    {
        ...
        steers_calibrate3_subscription_ = create_subscription<std_msgs::msg::Int32>(
            "/steers/calibrate3", rclcpp::QoS{0}, [this](std_msgs::msg::Int32::UniquePtr&& msg) {
                steers_calibrate3_subscription_callback(std::move(msg));
            });

            register_output("/chassis/control_velocity", chassis_control_velocity_);
            register_output("/clip/turner/target_angle", control_torques_[0]);
            register_output("/rack/suction_telescopic/target_angle", control_torques_[1]);
            register_output("/rack/suction_updown/target_angle", control_torques_[2]);
            register_output("/rack/clip_updown/target_angle", control_torques_[3]);
...
private:
    ...
    void steers_calibrate3_subscription_callback(std_msgs::msg::Int32::UniquePtr) {
        RCLCPP_INFO(
            get_logger(), "[steer calibration] New front offset: %d",
            bottom_board_->chassis_steer_motors_[0].calibrate_zero_point());
        RCLCPP_INFO(
            get_logger(), "[steer calibration] New left back offset: %d",
            bottom_board_->chassis_steer_motors_[1].calibrate_zero_point());
        ...
        private:
            ...
            struct REMOTE_Data_t{
                ...
                void remoteHandler(REMOTE_Data_t remoteData){
                    ...
                    // ROS_INFO("handler ");
                using GpioCmd = librmcs::client::GpioCommand;
...
class BottomBoard final : private librmcs::client::DMH7Board 
{
    public:
        ...
        explicit BottomBoard(
            ...
            , clip_turner_(test, test_command, "/clip/turner")
            , rack_suction_telescopic_(test, test_command, "/rack/suction_telescopic")
            , rack_suction_updown_(test, test_command, "/rack/suction_updown")
            , rack_clip_updown_(test, test_command, "/rack/clip_updown")
            , chassis_wheel_motors_(
                {test, test_command, "/chassis/front_wheel"},
            ...
            , chassis_steer_motors_(
                {test, test_command, "/chassis/front_steering"},
            ...
            std::cout << "=== Event thread ENDED ===" << std::endl;})
            {
                ...
                rack_suction_updown_.configure(
                    device::DjiMotor::Config{device::DjiMotor::Type::M3508}
                        .enable_multi_turn_angle()
                        .set_reversed()
                        // .set_reduction_ratio()
                        );
                rack_clip_updown_.configure(
                    device::DjiMotor::Config{device::DjiMotor::Type::M3508}
                        .enable_multi_turn_angle()
                        .set_reversed()
                        // .set_reduction_ratio()
...
/*                 for (auto& motor : chassis_wheel_motors_)
        ...
        void update() {
            ...
            clip_turner_.update_status();
            rack_suction_updown_.update_status();
            rack_clip_updown_.update_status();
...
/*                double rpm_A = chassis_wheel_motors_[i].generate_command(); 
            ...
            if (can_transmission_mode) {
                ...
                transmit_buffer_.trigger_transmission();
                can_commands[0] = clip_turner_.generate_command();
                can_commands[1] = rack_suction_telescopic_.generate_command();
                can_commands[2] = rack_suction_updown_.generate_command();
                can_commands[3] = rack_clip_updown_.generate_command();
    ...
    private:
        ...
        device::DjiMotor clip_turner_;
        device::DjiMotor rack_suction_telescopic_;
        device::DjiMotor rack_suction_updown_;
        device::DjiMotor rack_clip_updown_;
        device::VescMotor chassis_wheel_motors_[3];
        device::DjiMotor chassis_steer_motors_[3];

        uint64_t last_gpio_sent_[5] = {0};
    ...
    OutputInterface<double> control_torques_[4];
    uint16_t f4_commands[4];
    uint64_t gpio[5] = {0};

    rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr steers_calibrate_subscription_;
    rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr steers_calibrate3_subscription_;
};
}
