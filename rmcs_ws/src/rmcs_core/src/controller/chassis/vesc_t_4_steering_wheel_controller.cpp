...
private:
    ...
    Eigen::Vector3d calculate_chassis_velocity(
        ...
        velocity.x() = one_quarter_r * wheel_velocities.dot(steering_status.cos_angle);    //dot为点cheng
        ...
        velocity.z() = -one_quarter_r / vehicle_radius_
                     * (-wheel_velocities[0] * steering_status.sin_angle[0]
                        + wheel_velocities[1] * steering_status.cos_angle[1]                     //应该是将旋转的轮子的角度负值消掉因为只是看角速度     rad/s
    ...
    Eigen::Vector3d calculate_chassis_control_velocity() {
        ...
        chassis_control_velocity.head<2>() =
            Eigen::Rotation2Dd(-std::numbers::pi / 4) * chassis_control_velocity.head<2>();       //使x向为前
    ...
    Eigen::Vector3d calculate_chassis_control_acceleration(             
        ...
        const double& angular_control_velocity = chassis_control_velocity[2];        //rad/s
    ...
    Eigen::Vector4d calculate_wheel_pid_torques(                                 //pid控制器计算轮子角速
    ...
    Eigen::Vector3d constrain_chassis_control_acceleration(
        ...
        if (translational_acceleration_max > 0.0)
            translational_acceleration_direction /= translational_acceleration_max;        //向量归一化
    ...
    Eigen::Vector4d calculate_steering_control_torques(
        ...
        for (int i = 0; i < steering_control_velocities.size(); ++i) {
            if (dot_r_squared[i] > 1e-2) {
                ...
                auto x = ax - vehicle_radius_ * (az * sin_varphi_[i] + 0 * cos_varphi_[i]);     //加速度
    ...
    Eigen::Vector3d odom_to_base_link_vector(Eigen::Vector3d vector) const {
        vector.head<2>() = Eigen::Rotation2Dd(-chassis_yaw_angle_imu_) * vector.head<2>();                      //不是将odom转换到base_link上，而是将odom上的向量以base_link的基坐标系表示， Eigen::Rotation2Dd(-chassis_yaw_angle_imu_)等于一个逆矩阵
        return vector;
    }
